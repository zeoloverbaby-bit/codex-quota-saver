...
- secrets：`.local.env` chmod 600（POSIX）/ icacls 当前用户（Windows）；OAuth 密码与上游 token 经环境变量注入（进程命令行不可见）
- 重启免疫：OAuth 客户端注册表 + 签名密钥落盘 `oauth_state.json`（600），重启桥授权不失效；删除该文件 = 撤销全部已发 token
