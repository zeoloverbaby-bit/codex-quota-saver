...
12. **连接器被「运行时禁用」，但服务器一切正常**：ChatGPT 平台侧的连接器状态。先试会话级恢复：开新对话 + 放慢调用节奏 + 等 3-5 分钟冷却；**若新对话仍报 `Resource not found` 或 `The ... tool has been disabled`，禁用已升级为连接器级**（2026-08-18 实测，见坑 20）——直接删除连接器重建。别为此改服务器——乱改只会引入新变量。
...
1. **开个新对话重试**——多数会话级问题新对话清零；**若 GPT 报 `The ... tool has been disabled`，直接跳第 3 步**（那是连接器级禁用，新对话无效）
...
3. 还挂 → **删除连接器并重建**（ChatGPT 侧操作；本地桥配置完全不受影响，重建后走一次 OAuth 授权即可——实测 V2 被平台禁用后重连无效，重建 V3 即恢复）
4. 还挂 → **停下，取证**（冒烟三连 + 流量检查）。服务器侧全绿就别动服务器——问题在平台侧
5. 兜底：GPT 把 `.codex/next-step.md` 全文输出 → 你人工落盘，交接永不阻塞
